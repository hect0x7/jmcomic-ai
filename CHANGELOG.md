...
### Changed
- 📦 **上游依赖基线升级**：将 `jmcomic` 支持基线提升至 `>=2.7.4,<3.0.0`，使用下载清单与真实产物路径。
- 📋 **下载结果完整交接**：`download_album` 与 `download_photo` 新增实际图片路径、按扩展名分组的插件导出文件和下载耗时；下载目录改为读取上游完成后的真实结果，章节图片数按实际清单统计。
- 📄 **真实分页信息**：搜索、分类浏览和评论工具透传上游返回的当前页码，错误响应也保留请求页码。
- 🖥️ **终端进度配置同步**：Option Schema 支持上游 `download_progress` 插件的日志文件和终端日志行数；MCP 下载仍使用 Context 进度通知和任务日志。

### Added
- 🌐 **全站评论读取**：新增只读 MCP 工具 `get_forum_comments` 与 Skills 脚本 `forum_comments.py`，支持分页获取全站最新评论及其来源本子。

### Fixed
- 🧭 **下载路径预估偏差**：不再在下载前额外请求本子详情或根据目录规则预测返回路径，避免自定义规则、缓存及插件处理后返回值与真实落盘内容不一致。
- 📦 **后处理兼容 2.7.4**：ZIP、PDF 和长图插件可登记实际生成文件，`post_process` 返回完整 `output_paths`，不再因缺少新版登记接口而在文件生成后误报失败。
- 🧪 **MCP 集成测试假阳性**：下载和后处理测试现在校验业务状态，后处理还会确认返回的真实产物存在。
...
### Added
- 🖼️ **封面自定义输出目录**：`download_cover` 新增可选 `output_dir`，封面脚本支持任意输出目录并自动创建缺失的父目录；未指定时仍保存到默认 `covers/` 目录。

### Changed
- 🌐 **Skills 管理英文交互**：`jmai skills install/uninstall` 的提示与确认信息固定使用英文，不提供语言切换功能。
- 📊 **CSV 动态列导出**：搜索与榜单 CSV 保持核心列顺序，并自动追加整批结果中出现的扩展字段，兼容 `likes` 及后续新增字段。
- 📁 **输出目录自动创建**：本子详情、搜索、榜单和配置转换在写文件前自动创建嵌套父目录；相对路径继续以当前工作目录为基准。
- 🖼️ **README 架构图更新**：移除 Mermaid 架构图，替换为重新生成的横版双路线架构图片。
- 🚀 **Changelog 驱动发布文案**：发版提交只负责触发发布和标识版本；Actions 从对应的 Changelog 版本段生成 GitHub Release 正文，并在版本段缺失、重复、空内容或日期格式错误时阻止发布。
- 📝 **版本日志直接归档**：移除 `Unreleased` 占位与迁移流程，版本号确定后直接维护对应的带日期版本段落。

### Fixed
- 🧾 **本子详情脚本计数格式化**：`album_info.py` 兼容 JMComic API 返回的字符串类型点赞数和浏览数，避免控制台摘要因千分位格式化抛出 `ValueError`，并增加脚本级回归测试。
- 🛡️ **Skills 软链接卸载保护**：检测到外部管理的 Skill 软链接时仅提示并跳过，不删除链接或其目标源码。
- 🚦 **脚本失败退出状态**：批量下载、章节下载、封面下载、本子详情、榜单、诊断和后处理脚本在全部失败或部分失败时返回非零退出码，避免 Agent 与 CI 误判成功。
- ⚙️ **配置规则摘要崩溃**：`validate_config.py` 改用 `DirRule.rule_dsl` 读取目录规则，有效配置不再因错误调用 `.get()` 崩溃。
- 📦 **依赖导入错误误报**：Skills 脚本分别检查 `jmcomic` 与 `jmcomic_ai`，并区分主包未安装、传递依赖缺失和其他导入失败，给出对应安装提示。
- 🩺 **域名发现诊断信息**：Doctor 保留并输出动态域名发现的原始异常，不再仅显示笼统的失败提示。
- 🛑 **异步取消二次崩溃**：批量本子与章节下载正确传播 `CancelledError`，不再把取消对象当字典访问而抛出二次异常。
- 🔢 **批量 ID 文件解析**：先过滤空行及带缩进的注释，再使用 `JmcomicText.parse_to_jm_id` 统一解析纯数字、`JM` 前缀和链接格式。
- 🌐 **Doctor 非业务链接过滤**：域名诊断跳过上游返回的 `t.me` 发布链接，同时保留其他候选值参与真实可用性检查。
